...
import { Container, Paper, Typography, Box, IconButton, Chip, Stack } from '@mui/material'
...
const generateMockProducts = (t: (key: string) => string): Product[] => {
  ...
  return Array.from({ length: 50 }, (_, i) => ({
    id: i + 1,
    name: t('products.product').replace('{{id}}', String(i + 1)),
...
export default function ProductsPage() {
  ...
  const handleSaveProduct = (product: Partial<Product>) => {
    if (selectedProduct) {
      setProducts(
        products.map((p) => (p.id === selectedProduct.id ? { ...p, ...product } : p))
      )
    } else {
            ...
            >
              <Box>
                ...
                <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
                  {t('products.totalProducts').replace('{{count}}', String(products.length))}
                </Typography>
              </Box>
              <Button
                color="primary"
